describe ManageIQ::Providers::Foreman::ConfigurationManager::Refresher do
  ...
  context "#refresh" do
    ...
    it "graph refresh and classic refresh identical" do
      stub_settings_merge(:ems_refresh => {:foreman_configuration => {:inventory_object_refresh => false}})

      full_refresh
      before = serialize_inventory

      stub_settings_merge(:ems_refresh => {:foreman_configuration => {:inventory_object_refresh => true}})
    ...
    [{:inventory_object_refresh => false}, {:inventory_object_refresh => true}].each do |setting|
      context "with settings #{setting}" do
        before { stub_settings_merge(:ems_refresh => {:foreman_configuration => setting}) }
      ...
      def full_refresh
        ...
        VCR.use_cassette("#{described_class.name.underscore}_api_v2", :allow_unused_http_interactions => true) do
